...
#define SRAM_N_CE1 SRAM_30
#define SRAM_N_OE SRAM_32
#define SRAM_CE2 SRAM_36
#define SRAM_N_WE SRAM_37

#define DELAY_US 1

#define ADDRESS_PIN_COUNT 13
int address_pins[ADDRESS_PIN_COUNT] = { SRAM_A0, SRAM_A1, SRAM_A2, SRAM_A3, SRAM_A4, SRAM_A5, SRAM_A6, SRAM_A7, SRAM_A8, SRAM_A9, SRAM_A10, SRAM_A11, SRAM_A12 };
...
void set_default_signals() {
	...
	pinMode(SRAM_N_CE1, OUTPUT);
	digitalWrite(SRAM_N_CE1, LOW);

	pinMode(SRAM_N_OE, OUTPUT);
	digitalWrite(SRAM_N_OE, LOW);

	pinMode(SRAM_CE2, OUTPUT);
	digitalWrite(SRAM_CE2, LOW);

	pinMode(SRAM_N_WE, OUTPUT);
	digitalWrite(SRAM_N_WE, LOW);

	chip_enable(1);
	enable_output(1);
	write_enable(1); // CHANGED FROM 1 to 0 !!!!
}
...
void print_menu() {
	...
	Serial.println("---------------------------------");
	Serial.println("");
	Serial.println("Current layout: LC3564S-10 (13 address bits, 8 data bits)");
	Serial.println("");
	...
	Serial.println("\t 6: Run RAM spy mode!");
	Serial.println("");
	Serial.println("---------------------------------");
...
void ram_spy() {
	Serial.println("Running RAM spy mode (in development)!");
	Serial.println("Press 'c' to cancel!");

	set_all_input();

	attachInterrupt(digitalPinToInterrupt(SRAM_N_WE), spy_interrupt, CHANGE);//write enable up -> write performed
	attachInterrupt(digitalPinToInterrupt(SRAM_N_CE1), spy_interrupt, CHANGE);//
	attachInterrupt(digitalPinToInterrupt(SRAM_N_OE), spy_interrupt, CHANGE);//
	attachInterrupt(digitalPinToInterrupt(SRAM_CE2), spy_interrupt, CHANGE);//

	while (Serial.read() != 'c') {
		if (should_perform_spyread) {
			perform_spy_read();
			should_perform_spyread = false;
		}
		//Serial.println(intteruptcount);
		//delay(500);
	}

	detachInterrupt(digitalPinToInterrupt(SRAM_N_WE));
	detachInterrupt(digitalPinToInterrupt(SRAM_N_CE1));
	detachInterrupt(digitalPinToInterrupt(SRAM_N_OE));
	detachInterrupt(digitalPinToInterrupt(SRAM_CE2));

	Serial.println("Spying aborted!");
		
	set_default_signals();
}

void perform_spy_read() {
	uint8_t data = spy_data();
	uint16_t address = spy_read_address();
	bool enable = spy_read_enable();

	if (enable)Serial.print(" ENABLED ");
...
bool spy_read_enable() {
	return (digitalRead(SRAM_CE2) && !digitalRead(SRAM_N_CE1));
}

int spy_read_address() {
	int address_read;

	for (int i = 0; i < ADDRESS_PIN_COUNT; i++) {
		address_read |= digitalRead(address_pins[ADDRESS_PIN_COUNT - i - 1]);
		address_read << 1;
	}
...
int spy_data() {
	...
	for (int i = 0; i < DATA_PIN_COUNT; i++) {
		data_read |= digitalRead(address_pins[DATA_PIN_COUNT - i - 1]);
		data_read << 1;
	}
...
void chip_enable(bool on) {
	digitalWrite(SRAM_N_CE1, !on);
	digitalWrite(SRAM_CE2, on);
}
